""" 
Custom Environment for Double-Pendulum Relaxed by a Cart (DPRC)

In "stable_baselines3", a custom environment is defined via a class. Here we
create an environment which can be used to train an agent to control the force 
exerted by a cart and lead a chaotically-moving double-pendulum into a relaxed
state. The pendulums are initially set into a a pre-defined position (default 
position is set to pi/2 radians with respect to vertical.
"""
...
# DEFAULT PARAMETERS:
initial_position_pendulum1 = np.pi/2
initial_position_pendulum2 = np.pi/2
# Physical parameters
...
# Action Parameters
t_action = 0.0075    # sec     Duration of "constant" action
num_calcs = 3      # number of calculations performed by solver per timestep
t = np.linspace(0, t_action, num_calcs)
# Success/Fail Parameters
t_out = 5       # sec  Timeout
hold  = 1      # sec  Time after which agent gets more reward if pendulum inv
stretch = 0.925  # % of pendulum's vertical stretch 
...
def relax_check(time_held, hold_time_goal, pen1_angle, pen2_angle):
    ycoord_pen2 = L1*np.cos(pen1_angle) + L2*np.cos(pen2_angle)          
    if ycoord_pen2 <= -(L1 + L2) * (1-stretch):
...
class DPICenv(gym.Env):
    """
    This class defines the custom enviornment for a DPRC. See comments and 
    descriptions of definitions below.
    """
    ...
    def __init__(self):     
        ...
        self.u_high = 10 * self.m0 * self.g   # N        Maximum control force
        self.xmax_bd = 5*self.maxL          # m        Right bound of horizontal movement
    ...
    def step(self, u): 	
        """
        Most important part of the custom environment. This definition generates
        the agent's the observation and defines its reward function.

        Returns
        -------
        Observation, reward, and whether the agent "failed" (e.g., if the cart
        goes out of bounds or its pendulum's angular velocity is too high).
        ...
        if relax_check(self.time_held, self.hold_time_goal, th1, th2) == 2:
            self.time_held = self.time_held + self.t_action
            self.inv = True
        elif relax_check(self.time_held, self.hold_time_goal, th1, th2) == 1:
            self.time_held = self.time_held + self.t_action
        elif relax_check(self.time_held, self.hold_time_goal, th1, th2) == 0:
            self.time_held = 0
        ...
        self.reward = (100*(np.cos(np.pi * th0 / self.xmax_bd))
        #    -(th0 - self.xmax_bd)**2 - self.xmax_bd**2
                        - dth0**2/10
                        - (dth1**2 + dth2**2)
                        + (th1**2  + th2**2)*2
                        - u_action**2/100
        ...
        if out_of_bounds(th0, self.xmax_bd) == 1:
            self.reward = self.reward - 1e3*(abs(th0) + dth1**2 + dth2**2)
            self.done = True
        elif timeout(self.time_elapsed) == 1:
            self.done = True
    ...
    def reset(self):
        """
        Reset method is herein defined.
        This is where the question "what's the state of the cart every time
        it resets?" is answered.

        Returns
        -------
        Initial observation of agent.

        """
        # Initializing DPRC conditions
        self.u = 0
        self.time_elapsed = 0
        self.time_held = 0
        
        
        # Initial state of DPRC
        self.cart_pos = 0.
        self.cart_speed = 0.
        self.pen1_ang = initial_position_pendulum1/np.pi
        self.pen1_angVel = 0.
        self.pen2_ang = initial_position_pendulum2/np.pi
        self.pen2_angVel = 0.
